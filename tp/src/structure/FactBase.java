...
public class FactBase
{
	...
	/**
	 * @param j le rang
	 * @return une chaÃ®ne de caractÃ¨re contenant la liste des faits d'indice supÃ©rieur ou Ã©gal Ã  j (utilisÃ© pour obtenir la liste des faits dÃ©duits)
	 */
	public String afficheSupRang(int j)
	{
		String s="";
		if (atoms.size()<j) s="Pas de nouveaux faits !";
		else{
			for(int i=j;i<atoms.size()-1;i++)
				s+=	atoms.get(i)+" ; ";	
			s+= atoms.get(atoms.size()-1)+"\n";
		}
		return s;
