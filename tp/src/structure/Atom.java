...
public class Atom
{
	...
	public Atom(Atom a,Substitution s){
		predicate=a.predicate;
		args=new ArrayList<Term>();
		for(Term t:a.args){
			args.add(new Term(s.getConst(t)));
		}
	...
	public int getNbVar(){
		int res=0;
		ArrayList<Term> at=new ArrayList<Term>();
		for (Term t:args){
			if (t.isVariable()&&!t.isContain(at)){
				res++;
				at.add(t);
			}
		}
	...
	public boolean isContain(ArrayList<Atom> al){
			for(Atom a:al){
				if(this.equalsA(a)){
					return true;
				}
